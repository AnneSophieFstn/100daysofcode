...
export class WeatherService {
  ...
  constructor(lat: number, lng: number) {
    this.baseUrl = `https://api.tomorrow.io/v4/weather/forecast?location=42.3478,-71.0466&apikey=${this.API_KEY}&units=imperial&timesteps=1d&timesteps=1h`;
    this.nwsUrl = `https://api.weather.gov/points/42.3478,-71.0466`;
  }
  ...
  private async fetchWeatherPoints(): Promise<WeatherPointResponse | null> {
    try {
      console.log("Fetching weather data from URL:", this.nwsUrl);
